...
function Found() {
	...
	useEffect(() => {
		const getLatestSubmissions = async () => {
			const data = await fetchLatestSubmissions("found");
			setHits(data);
		};
	...
	const search = _.debounce(async (event) => {
		...
		setSearched(search);
		if (search.length > 2) {
			const data = await searchSubmissionsByFullText("found", search);
			setHits(data);
		}
	}, 1000);
	...
	const searchByCategory = async (category) => {
		if (category.value === "All") {
			...
			const data = await searchSubmissionsByCategory("found", category.value);
			searchInputRef.current.value = "";
			setSearched("");
			setSelectedCat(category);
			setHits(data);
